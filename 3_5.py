...
X_train, X_test, Y_train, Y_test = np.load('H:/1. Research/3. PoDoc_Kim/1. CoCr_AI design/2. Python Code/n_comp_n_hatch_CODE_NPY/3_5.npy', allow_pickle = True)
print(X_train.shape)
...
model.add(Conv1D(32, 3, padding="same", input_shape=(3, 1), activation='relu'))
...
model.save('H:/1. Research/3. PoDoc_Kim/1. CoCr_AI design/2. Python Code/model/3_5.h5', overwrite=True)

# Confusion matrix

from sklearn.metrics import confusion_matrix, plot_confusion_matrix #교차표 그리기   

prediction = model.predict(X_test)

cfm_3_5=confusion_matrix(Y_test.argmax(axis=1), prediction.argmax(axis=1))

import seaborn as sns
sns.set(font_scale=2.0)
ax = sns.heatmap(cfm_3_5, annot=True, cmap='Blues', fmt='g')

ax.set_title('3_comp_5_hatching');
